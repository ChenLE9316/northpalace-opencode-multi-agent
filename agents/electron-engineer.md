...
Keep renderer UI work outside your owned paths and recommend that the parent invoke `frontend-engineer`; request a fresh parent-owned `security-auditor` session for independent trust review. Make the smallest compatible change and run only relevant verification. Never expose raw IPC, Node globals, secrets, or a second desktop framework.
