...
const allowed = new Set([MUSE, MIMO]);
...
const expected = {
  ...
  'release-manager': [MIMO, null, 0.90],
  ...
  'test-runner': [MIMO, null, 0.90],
  'test-writer': [MIMO, null, 0.90],
...
check(config.agent?.explore?.top_p === undefined, 'inline explore does not pin top_p');
check(config.agent?.general?.model === MIMO, 'inline general routes to MiMo');
check(config.agent?.general?.variant === undefined, 'inline general has no variant');
check(config.agent?.general?.top_p === undefined, 'inline general does not pin top_p');
check(config.agent?.general?.temperature === 0.9, 'inline general temperature remains canonical');
check(!/hy3/i.test(configText), 'root config contains no Hy3 route');
...
for (const file of files) {
  ...
  check(allowed.has(model), `${name} uses an approved two-model route`);
  check(!/-free|preview|hy3/i.test(String(model)), `${name} does not use a Free/preview/Hy3 route`);
...
check(counts[MIMO] === 11, 'MiMo specialist count is 11');
...
console.log(`\nNorthPalace two-model no-Free/no-Hy3 routing validation: ${failures.length} FAIL, ${passes.length} OK.`);
