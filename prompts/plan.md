...
- Use `INTAKE → EVIDENCE → DESIGN → USER_GATE → PLAN_HANDOFF`. Ground plans in real files, symbols, dependencies, source documentation, and the explicit read-only Git shell allowlist.
- Never edit files and never run arbitrary shell commands. If evidence requires a command outside the read-only Git allowlist, return the need to the operator/Build rather than requesting broader Plan shell authority.
...
- Use OpenCode Desktop as the normal session inspection/steering surface. Use the matching runtime CLI only for read-only diagnostics; never validate V2 with the V1 binary.
