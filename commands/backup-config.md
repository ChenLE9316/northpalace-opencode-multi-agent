...
5. List backups older than the newest 14, but do not delete/rotate them automatically. Raw deletion/rotation routes remain hard-denied; a narrower cleanup command may be `ask`, but backup rotation is still a separate operator-owned action/policy choice.
